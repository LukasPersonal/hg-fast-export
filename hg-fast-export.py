...
def split_dict(
    dleft, dright, left_stack=[], center_stack=[], right_stack=[], match=file_mismatch
):
    ...
    for left in dleft.keys():
        ...
        elif match(dleft[left], right) or gitmode(dleft.flags(left)) != gitmode(
            dright.flags(left)
        ):
            # we have it but checksums mismatch: add to center set
            center_stack.append(left)
    ...
    return left_stack, center_stack, right_stack
...
def refresh_hg_submodule(name, subrepo_info):
    ...
    if name not in subrepo_cache:
        subrepo_cache[name] = (
            load_cache(gitRepoLocation + b"/hg2git-mapping"),
            load_cache(gitRepoLocation + b"/hg2git-marks", lambda s: int(s) - 1),
        )
...
def export_file_contents(ctx, manifest, files, hgtags, encoding="", plugins={}):
    ...
    for file in files:
        if not is_submodules_refreshed and (
            file == b".hgsub" or file == b".hgsubstate"
        ):
...
def sanitize_name(name, what="branch", mapping={}):
    ...
    # Grab the name in btye format
    term = mapping.get(name, name)
    # Set the pattern to exclude non alphanumeric and a few others
    pattern = re.compile(b"([^a-zA-Z0-9-_]|\.\.)")  # noqa
    # pattern = re.compile(b"([\[ ~^:?\\\\*]|\.\.)")
    # Update name to remove bad patterns
    term = pattern.sub(b"", term)  # noqa
    # Set the pattern to include //
    pattern = re.compile(b"(\/\/)")  # noqa
    # pattern = re.compile(b"([\[ ~^:?\\\\*]|\.\.)")
    # Update name to remove bad patterns
    term = pattern.sub(b"", term)  # noqa
    # check if first char is not acceptable
    if term[0] in (b"/", b"."):
        term = term[1:] + b"_"
    # check if first char is not acceptable
    if term[0] in (b"-", b"_"):
        term = term[1:] + b""
    # check if last char is not acceptable
    if term[-1:] in (b"/", b"."):
        term = term[:-1] + b"_"
    # check if last char is not acceptable
    if term[-1:] in (b"-", b"_"):
        term = term[:-1] + b""
    term = b"/".join([dot(s) for s in term.split(b"/")])
    decode_string = term.decode("utf-8").encode("ascii", "ignore")
    term = decode_string
    pattern = re.compile(b"_+")
    term = pattern.sub(b"_", term)
    # Debug print to see updated naming
    new_string = term.decode("utf-8", "ignore")
    sys.stderr.write("Value of new string: " + new_string + "\n")

    if term != name:
        stderr_buffer.write(
            b"Warning: sanitized %s [%s] to [%s]\n" % (what.encode(), name, term)
        )
    return term
...
def export_commit(
    ...
    (revnode, _, user, (time, timezone), files, desc, branch, extra) = get_changeset(
        ui, repo, revision, authors, encoding
    )
...
def export_note(ui, repo, revision, count, authors, encoding, is_first):
    (revnode, _, user, (time, timezone), _, _, _, _) = get_changeset(
        ui, repo, revision, authors, encoding
    )
    if repo[revnode].hidden():
        return count

    parents = [p for p in repo.changelog.parentrevs(revision) if p >= 0]  # noqa: F841
...
def export_tags(ui, repo, old_marks, mapping_cache, count, authors, tagsmap):
    ...
    for tag, node in list:  # noqa: F402
        ...
        if hexlify(node) not in mapping_cache:
            stderr_buffer.write(
                b"Tag %s refers to unseen node %s\n" % (tag, hexlify(node))
            )
            continue
        ...
        if ref is None:
            ...
            continue
        stderr_buffer.write(
            b"Exporting tag [%s] at [hg r%d] [git %s]\n" % (tag, rev, ref)
        )
...
def load_mapping(name, filename, mapping_is_raw):
    ...
    string_regexp = b'"(((\\.)|(\\")|[^"])*)"'
    quoted_regexp = re.compile(
        b"^" + string_regexp + b"[ ]*=[ ]*" + string_regexp + b"$"
    )
    ...
    def parse_quoted_line(line):
        ...
        return (
            process_unicode_escape_sequences(m.group(1)),
    ...
    for line in file.readlines():
        length += 1
        line = line.strip()
        if length == 1 and line[0:1] == b"#" and line == b"# quoted-escaped-strings":
            continue
        elif line == b"" or line[0:1] == b"#":
            continue
        mark = parse_raw_line(line) if mapping_is_raw else parse_quoted_line(line)
        if mark is None:
            sys.stderr.write(
...
def verify_heads(ui, repo, cache, force, ignore_unnamed_heads, branchesmap):
    ...
    line = [(-repo.changelog.rev(n), n, t) for t, n in branches.items()]
    line.sort()
    ...
    for h in repo.filtered(b"visible").heads():
        ...
        if t.get(branch, False):
            ...
            unnamed_heads = True
            if not force and not ignore_unnamed_heads:
                return False
        t[branch] = True
    if unnamed_heads and not force and not ignore_unnamed_heads:
        return False
    return True
...
def hg2git(
    ...
    if not verify_heads(
        ui, repo, heads_cache, force, ignore_unnamed_heads, branchesmap
    ):
        return 1
    ...
    if notes:
        for rev in range(min, max):
            c = export_note(
                ui, repo, rev, c, authors, encoding, rev == min and min != 0
            )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-m", "--max", type="int", dest="max", help="Maximum hg revision to import"
    )
    parser.add_option(
        "--mapping",
        dest="mappingfile",
    ...
    parser.add_option(
        ...
        dest="sob",
        default=False,
        help="Enable parsing Signed-off-by lines",
    )
    parser.add_option(
        "--hgtags",
        action="store_true",
        dest="hgtags",
        default=False,
        help="Enable exporting .hgtags files",
    )
    parser.add_option(
        "-A", "--authors", dest="authorfile", help="Read authormap from AUTHORFILE"
    )
    parser.add_option(
        "-B",
        "--branches",
    ...
    parser.add_option(
        "-T", "--tags", dest="tagsfile", help="Read tags map from TAGSFILE"
    )
    parser.add_option(
        "-f",
        "--force",
        ...
        dest="force",
        default=False,
        help="Ignore validation errors by force, implies --ignore-unnamed-heads",
    )
    parser.add_option(
        "--ignore-unnamed-heads",
        action="store_true",
        dest="ignore_unnamed_heads",
        default=False,
        help="Ignore unnamed head errors",
    )
    parser.add_option(
        "-M", "--default-branch", dest="default_branch", help="Set the default branch"
    )
    parser.add_option(
        "-o",
        "--origin",
    ...
    parser.add_option(
        ...
        dest="notes",
        default=False,
        help="Annotate commits with the hg hash as git notes in the hg namespace",
    )
    parser.add_option(
        "-e",
        dest="encoding",
    ...
    if options.subrepo_map:
        if not os.path.exists(options.subrepo_map):
            sys.stderr.write(
                "Subrepo mapping file not found %s\n" % options.subrepo_map
            )
            sys.exit(1)
        submodule_mappings = load_mapping(
            "subrepo mappings", options.subrepo_map, False
        )
    ...
    for plugin in plugins:
        ...
        if hasattr(plugin, "commit_message_filter") and callable(
            plugin.commit_message_filter
        ):
